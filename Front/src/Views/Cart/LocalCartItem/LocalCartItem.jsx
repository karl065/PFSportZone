...
import styles from "./LocalCartItem.module.css";
import useLocalCart from "../../../helpers/useLocalCart";

const LocalCartItem = ({ product }) => {
  const { id_inventory, article_name, selling_price, stock, image, cant } =
    product;

  const { addToLocalCart, removeFromLocalCart } = useLocalCart();

  const [selectedQuantity, setSelectedQuantity] = useState(cant);
  ...
  const updateCart = (newValue) => {
    ...
    setIsLoading(true);
    addToLocalCart(product, newValue);
    // * Solo para hacer como si fuera algo asíncrono por que quiero mostrar el LoadingSpinner.
    setTimeout(() => {
      setIsLoading(false);
    }, 200);
  };
  ...
  return (
    <div className={styles.product_container}>
      ...
      <p className={styles.price_per_amount}>${cant * selling_price}</p>
